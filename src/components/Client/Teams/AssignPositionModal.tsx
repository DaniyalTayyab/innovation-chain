...
function AssignPositionModal({ stateAction }: any) {
	const { isAssignPositionModalOpen, closeAssignPositionModal, MobileUser } =
		useMainContext();
	...
	useEffect(() => {
		setFormValidate({
			...formValidate,
			user_id: formData.user_id.length > 0 ? true : false,
	...
	const handleSubmit = async (e: any) => {
		...
		if (!formValidate.user_id) {
			toast.error(`there was an error. please fill in member id.`, {
				position: "top-left",
			});
			setLoading(false);
		} else if (!formValidate.name) {
			...
			await axios_auth(currentToken)
				.put(api_url + "/user/nodes/update", formData)
				.then((response: any) => {
	...
	return (
		<Modal
			...
			style={{ top: 100 }}
			footer={<div></div>}
			open={isAssignPositionModalOpen}
			onOk={closeAssignPositionModal}
			onCancel={closeAssignPositionModal}
		>
			<div className="flex flex-col space-y-4 py-3 px-5">
				...
				<div className="relative">
					...
					<input
						...
						autoComplete="off"
						aria-invalid={formValidate.user_id ? "false" : "true"}
						aria-describedby="uidnote"
						onFocus={() => setFormFocus({ ...formFocus, user_id: true })}
						onBlur={() => setFormFocus({ ...formFocus, user_id: false })}
					...
					<div
						id="uidnote"
						className={
							formFocus.user_id && formData.user_id && !formValidate.user_id
